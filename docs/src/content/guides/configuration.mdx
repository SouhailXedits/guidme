...
guidme is built to be highly configurable. You can configure the guidme globally, or per step. You can also configure the guidme on the fly, while it's running.

> guidme is written in TypeScript. Configuration options are mostly self-explanatory. Also, if you're using an IDE like WebStorm or VSCode, you'll get autocomplete and documentation for all the configuration options.

## Guidme Configuration

You can configure the guidme globally by passing the configuration object to the `guidme` call or by using the `setConfig` method. Given below are some of the available configuration options.
...
type Config = {
  ...
  //   - options.state: The current state of the guidme
  ...
  // Hooks to run before and after the guidme
...
>
> `onNextClick` and `onPrevClick` hooks can be configured at the step level as well. When configured at the guidme level, you control the navigation for all the steps. When configured at the step level, you control the navigation for that particular step only.

## Popover Configuration

The popover is the main UI element of guidme. It's the element that highlights the target element, and shows the step content. You can configure the popover globally, or per step. Given below are some of the available configuration options.
...
You can access the current state of the guidme by calling the `getState` method. It's also passed to the hooks and callbacks.

```typescript
type State = {
  // Whether the guidme is currently active or not
