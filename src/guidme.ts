...
export function guidme(options: Config = {}) {
  configure(options);
  ...
  function init() {
    ...
    document.body.classList.add("guidme-active", getConfig("animate") ? "guidme-fade" : "guidme-simple");
  ...
  function destroy(withOnDestroyStartedHook = true) {
    ...
    // the hook for when user calls `destroy`, guidme will get into infinite loop
    // not causing tour to be destroyed.
    if (withOnDestroyStartedHook && onDestroyStarted) {
      const isActiveDummyElement = !activeElement || activeElement?.id === "guidme-dummy-element";
    ...
    document.body.classList.remove("guidme-active", "guidme-fade", "guidme-simple");
    ...
    if (activeElement && activeStep) {
      const isActiveDummyElement = activeElement.id === "guidme-dummy-element";
      if (onDeselected) {
...
export type Guidme = ReturnType<typeof guidme>;
