...
function mountDummyElement(): Element {
  const existingDummy = document.getElementById("guidme-dummy-element");
  if (existingDummy) {
  ...
  element.id = "guidme-dummy-element";
...
export function highlight(step: GuidmeStep) {
  const { element } = step;
...
function transferHighlight(toElement: Element, toStep: GuidmeStep) {
  const duration = 400;
  ...
  const isToDummyElement = toElement.id === "guidme-dummy-element";
  const isFromDummyElement = fromElement.id === "guidme-dummy-element";
  ...
  fromElement.classList.remove("guidme-active-element", "guidme-no-interaction");
  ...
  if (disableActiveInteraction) {
    toElement.classList.add("guidme-no-interaction");
  }

  toElement.classList.add("guidme-active-element");
  toElement.setAttribute("aria-haspopup", "dialog");
  toElement.setAttribute("aria-expanded", "true");
  toElement.setAttribute("aria-controls", "guidme-popover-content");
}

export function destroyHighlight() {
  document.getElementById("guidme-dummy-element")?.remove();
  document.querySelectorAll(".guidme-active-element").forEach(element => {
    element.classList.remove("guidme-active-element", "guidme-no-interaction");
